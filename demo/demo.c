...
#define GATEWAY_PID  "/var/run/gateway.pid"
...
{
    FILE *fp = NULL;
    LOG_WRITE_PARAM log_param = {0};

    fp = fopen(GATEWAY_PID, "w");
    fprintf(fp, "%d", getpid());
    fclose(fp);
    ...
    #if 0
    char path[64] = "logInfo/config/info";
    node_t *node = NULL;
    process system_process = NULL;

    system_process = getProcess(root_node, path, &node);
    if(NULL == system_process)
    {
        log_error("system_process is NULL!");
        return -1;
    }
    log_warn("name:%s\n",node->servicename);
    (*system_process)(NULL, NULL);
    #endif
